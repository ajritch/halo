...
//        if let urlToReq = NSURL(string: "http://Annie-MacBook.local:8000/individuals") {
            ...
            do {
                request.HTTPBody = try NSJSONSerialization.dataWithJSONObject(["subject_id": subject_id, "first_name": first_name, "last_name": last_name, "gender": gender, "date_of_birth": date_of_birth, "ethnicity": ethnicity, "height_feet": height_feet, "height_inches": height_inches, "weight": weight], options: .PrettyPrinted)
                let session = NSURLSession.sharedSession()
